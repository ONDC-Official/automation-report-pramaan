...
function confirmMessageTests({ context, message } = {}, constants = {}) {
    try {
        ...
        const messageTestSuite = generateTests({ context, message }, confirmSchema, "Verification of Message for Confirm", constants);
...
module.exports = async function confirm({ context, message } = {}, logs = [],constants ={}) {
    ...
    try {
        ...
        console.log(err);
        testSuite.addTest(new Mocha.Test("confirm payload could not be verified due to some payload missing or internal error", function () {
