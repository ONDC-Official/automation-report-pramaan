...
module.exports = async function confirm({ context, message } = {}, previous_on_init_payment_id = "", settlementAmount = 0,  metaData = {},logs = [],constants ={}) {
    const testSuite = new Mocha.Suite('Confirm Request Verification');

    contextTests(context, testSuite, logs, { action: 'confirm', ...metaData });
    ...
    if (message?.order?.payments && message?.order?.payments.length > 0) {
        ...
        message?.order?.payments.forEach((payment, index) => {
            ...
            if (payment?.tags) {
                ...
                messageTestSuite.addTest(new Mocha.Test(`'message.order.payments[${index}].params' should include all the required keys`, function () {
