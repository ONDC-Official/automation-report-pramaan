...
module.exports = async function updatePayments({ context, message } = {}, logs = [], metaData = {}) {
    try {
        ...
        if (foreClosures && foreClosures.length > 0) {
            foreClosures.forEach((ele, index) => {
                if (ele?.time?.label === "FORECLOSURE") {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`Verify if 'message.order.payments[${index}].time.label' is 'FORECLOSURE'`, function () {
