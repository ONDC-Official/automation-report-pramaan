...
async function init({ context, message } = {}, step, previous_on_init_payment_id = "", testCaseId, logs = [],constants ={}) {
    ...
    try {
        ...
        messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] Verify the presence and type of 'message' object`, function () {
        ...
        if (message?.order?.items && message?.order?.items.length > 0) {
            message?.order.items.forEach((item, i) => {
                ...
                if (step === "II" || step === "III") {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] verify that 'message.order.items[${i}].xinput' is an object and has properties 'form' and 'form_response'`, function () {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] verify that 'message.order.items[${i}].xinput.form_response' is an object and has properties 'id' and 'submission_id'`, function () {
        ...
        if (message?.order?.payments && message?.order?.payments.length > 0) {
            ...
            message?.order?.payments.forEach((payment, index) => {
                ...
                if (payment?.tags) {
                    switch (step) {
                        case "I":
                            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] 'message.order.payments[${index}]' should have all the required keys`, function () {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] 'message.order.payments[${index}].params' should include all the required keys`, function () {
                    ...
                    const arr1 = [{ code: "BAP_TERMS" }];
                    const arr2 = [{ code: "BAP_TERMS" }, { code: "SETTLEMENT_TERMS" }];
                    let arr;
                    switch (step) {
                        case "I":
                        case "II":
                    ...
                    arr.forEach((ele) => {
                        ...
                        if (tagIndex !== -1) {
                            ...
                            const bapTermsArr1 = [{ code: "BUYER_FINDER_FEES_TYPE" }, { code: "BUYER_FINDER_FEES_PERCENTAGE" }, { code: "DELAY_INTEREST" }, { code: "STATIC_TERMS" }, { code: "SETTLEMENT_AMOUNT" }, { code: "SETTLEMENT_TYPE" }, { code: "OFFLINE_CONTRACT" }];
                            const bapTermsArr2 = [{ code: "BUYER_FINDER_FEES_TYPE" }, { code: "BUYER_FINDER_FEES_PERCENTAGE" }];
                            const settlementTermsArr = [{ code: "DELAY_INTEREST" }, { code: "STATIC_TERMS" }, { code: "SETTLEMENT_AMOUNT" }, { code: "SETTLEMENT_TYPE" }, { code: "OFFLINE_CONTRACT" }];
        ...
        testSuite.addTest(new Mocha.Test("init request failed to be verified because of some missing payload or some internal error", function () {
