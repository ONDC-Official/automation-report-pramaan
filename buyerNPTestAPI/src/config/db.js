...
const { MONGODB_URI, FLUSHPERIOD, BUYER_TESTING_URI } = require("../utils/env");
...
class Database {
    ...
    async connect() {
        try {
            ...
            if (!Report)
                await this.client.db.createCollection("reports");
    ...
    async getReports(test_id) {
        ...
        try {
            ...
            const result = await Promise.all(reports.map(async (report) => {
                ...
                // await fs.promises.rm(extractToDir, { recursive: true, force: true })
    ...
    //                 "Report Link": `${BUYER_TESTING_URI}/download-report/8704aef9-8985-4b8a-9c45-2321187d3889/${doc?.test_id}`,
    //                 "Logs Link": `${BUYER_TESTING_URI}/download-logs/8704aef9-8985-4b8a-9c45-2321187d3889/${doc?.transaction_id}`,
    //             }
        ...
        try {
            ...
            for (let collection of collections) {
                if (collection.name !== "reports") {
                    ...
                    console.log(`${result.deletedCount} documents were deleted from the '${collection.name}' collection.`);
                }
            }
        } catch (err) {
            console.log(err)
