...
module.exports = function contextTests(context = {}, constants = {}) {
    try {
        ...
        Object.keys(validSchema.properties).forEach(prop => {
            ...
            if (property?.type) {
                contextTestSuite.addTest(new Mocha.Test(`'[id: ${property?.id}_type] context.${prop}' should be a ${property.type}`, function () {
            ...
            if (property?.format) {
                const test = formatCheck({ value: context[prop], format: property.format, testName: `[id: ${property?.id}_format] context.${prop}` })
                if (test) {
