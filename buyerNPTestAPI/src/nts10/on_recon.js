    ...
    try {
        ...
        if (message?.orders && message?.orders?.length > 0) {
            message?.orders.forEach((order, orderIndex) => {
                ...
                if (order?.settlements && order?.settlements.length > 0) {
                    order?.settlements.forEach((settle, index) => {
                        ...
                        if (settle?.status !== "SETTLED") {
                            if (settle.amount.diff_value) {
                                messageTestSuite.addTest(new Mocha.Test(`'message.orders[${orderIndex}].settlements[${index}].amount.diff_value' should be a non-empty string`, function () {
                                    expect(settle.amount.diff_value).to.be.a('string').that.is.not.empty;
                                }));
                            }
                        }
                        ...
                        if (settle?.status !== "SETTLED") {
                            if (settle.commission.diff_value) {
                                messageTestSuite.addTest(new Mocha.Test(`'message.orders[${orderIndex}].settlements[${index}].commission.diff_value' should be a non-empty string`, function () {
                                    expect(settle.commission.diff_value).to.be.a('string').that.is.not.empty;
                                }));
                            }
                        }
                        ...
                        if (settle?.status !== "SETTLED") {
                            if (settle.withholding_amount.diff_value) {
                                messageTestSuite.addTest(new Mocha.Test(`'message.orders[${orderIndex}].settlements[${index}].withholding_amount.diff_value' should be a non-empty string`, function () {
                                    expect(settle.withholding_amount.diff_value).to.be.a('string').that.is.not.empty;
                                }));
                            }
                        }
                        ...
                        if (settle?.status !== "SETTLED") {
                            if (settle.tcs.diff_value) {
                                messageTestSuite.addTest(new Mocha.Test(`'message.orders[${orderIndex}].settlements[${index}].tcs.diff_value' should be a non-empty string`, function () {
                                    expect(settle.tcs.diff_value).to.be.a('string').that.is.not.empty;
                                }));
                            }
                        }
                        ...
                        if (settle?.status !== "SETTLED") {
                            if (settle.tds.diff_value) {
                                messageTestSuite.addTest(new Mocha.Test(`'message.orders[${orderIndex}].settlements[${index}].tds.diff_value' should be a non-empty string`, function () {
                                    expect(settle.tds.diff_value).to.be.a('string').that.is.not.empty;
                                }));
                            }
                        }
                ...
                messageTestSuite.addTest(new Mocha.Test(`'message.orders[${orderIndex}].recon_accord' should be an boolean`, function () {
                    expect(order.recon_accord).to.be.an('boolean');
                }));
            }
            )
        }
...
async function on_recon({ context, message } = {}, step, version = "") {
    try {
        const testSuite = new Mocha.Suite(`on_recon (${step}) request verification`);

        contextTests(context, "on_recon", testSuite, version);
