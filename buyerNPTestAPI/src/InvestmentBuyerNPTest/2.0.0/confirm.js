...
module.exports = async function confirm({ context, message } = {}, testCaseId, flowId, logs = [],constants ={}) {
    try {
        ...
        messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order' should have a property named 'id' which is an object`, function () {
        ...
        if (message?.order?.items && message?.order?.items.length > 0) {
            message.order.items.forEach((item, itemIndex) => {
                ...
                if (flowId !== "INVESTMENT_7") {
                    messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] 'message.order.items[${itemIndex}].payment_ids' should be an array`, function () {
                        expect(item).to.have.property("payment_ids").that.is.an("array");
                    }));

                    if (item?.payment_ids && item.payment_ids.length > 0) {
                        item.payment_ids.forEach((paymentId, paymentIndex) => {
                            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}] 'message.order.items[${itemIndex}].payment_ids[${paymentIndex}]' should be a string`, function () {
        ...
        if (message?.order?.fulfillments && message?.order?.fulfillments.length > 0) {
            message.order.fulfillments.forEach((fulfillment, fulfillmentIndex) => {
                ...
                fulfillment.agent.organization.creds.forEach((cred, credIndex) => {
                    ...
                    if (flowId !== "INVESTMENT_5" && flowId !== "INVESTMENT_7" && flowId !== "INVESTMENT_9" && flowId !== "INVESTMENT_20" && flowId !== "INVESTMENT_6") {
        ...
        // Verify 'message.order.xinput' properties
        if (flowId !== "INVESTMENT_1" && flowId !== "INVESTMENT_2" && flowId !== "INVESTMENT_3" && flowId !== "INVESTMENT_5" && flowId !== "INVESTMENT_6" && flowId !== "INVESTMENT_7" && flowId !== "INVESTMENT_9" && flowId !== "INVESTMENT_20" && flowId !== "INVESTMENT_4") {

            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order' should have a property named 'xinput' which is an object`, function () {
                expect(message.order).to.have.property("xinput").that.is.an("object");
            }));

            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order.xinput' should have a property 'form' which is an object`, function () {
                expect(message.order.xinput).to.have.property("form").that.is.an("object");
            }));

            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order.xinput.form' should have a property 'id' which is a string (OPTIONAL)`, function () {
                expect(message.order.xinput.form).to.have.property("id").that.is.a("string");
            }));

            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order.xinput' should have a property 'form_response' which is an object`, function () {
                expect(message.order.xinput).to.have.property("form_response").that.is.an("object");
            }));

            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order.xinput.form_response' should have properties 'status'`, function () {
                expect(message.order.xinput.form_response).to.have.property("status").that.is.a("string");
            }));

            messageTestSuite.addTest(new Mocha.Test(`[${testCaseId}${getNextTestcaseId()}]'message.order.xinput.form_response' should have properties 'submission_id (OPTIONAL)'`, function () {
        ...
        if (flowId !== "INVESTMENT_7") {
            ...
            if (message?.order?.payments && message?.order?.payments.length > 0) {
                message.order.payments.forEach((payment, paymentIndex) => {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.payments[${paymentIndex}].status' which is a string and should be equal to 'PAID' (OPTIONAL)`, function () {
                        expect(payment.status).to.exist.and.to.be.a("string").to.be.oneOf(['PAID', "NOT-PAID"]);
                    }));
