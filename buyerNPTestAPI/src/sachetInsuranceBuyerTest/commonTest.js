...
function providerTests(message, messageTestSuite) {
  try {
    ...
    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of'message.intent.provider' should have a property named 'tags' which is an array`, function () {
      expect(message.intent.provider).to.have.property("tags").that.is.an("array");
    }));


    if (message?.intent?.provider?.tags && message?.intent?.provider?.tags.length > 0) {
      message?.intent?.provider?.tags.forEach((tag, tagIndex) => {
        messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.tags[${tagIndex}]' should have properties 'descriptor', and 'list'`, function () {
          expect(tag).to.have.property("descriptor").that.is.an("object");
          expect(tag).to.have.property("list").that.is.an("array");
        }));

        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of'message.intent.provider.tags[${tagIndex}].descriptor' should have a property named 'code' which is a string`, function () {
          expect(tag.descriptor).to.have.property("code").that.is.a("string");
        }));

        messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.tags[${tagIndex}].list' should be a non-empty array`, function () {
          expect(tag.list).to.be.an("array").that.is.not.empty;
        }));

        tag.list.forEach((listItem, listItemIndex) => {
          messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.intent.provider.tags[${tagIndex}].list[${listItemIndex}]' should have properties 'descriptor' and 'value'`, function () {
            expect(listItem).to.have.property("descriptor").that.is.an("object");
            expect(listItem).to.have.property("value").that.is.a("string");
          }));
          messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.tags[${tagIndex}].list[${listItemIndex}].descriptor' should have a property named 'code' which is a string`, function () {
            expect(listItem.descriptor).to.have.property("code").that.is.a("string");
          }));
          messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.intent.provider.tags[${tagIndex}].list[${listItemIndex}].descriptor.code' should be equal to '${listItem.descriptor.code}'`, function () {
            expect(listItem.descriptor.code).to.equal(listItem.descriptor.code);
          }));
          messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.intent.provider.tags[${tagIndex}].list[${listItemIndex}].value' should be a string that is not empty`, function () {
    ...
    if (message?.intent?.provider?.items && message?.intent?.provider?.items.length > 0) {
      message?.intent?.provider?.items.forEach((item, i) => {
        ...
        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of'message.intent.provider.items[${i}]' should have a property named 'tags' which is an array`, function () {
          expect(item).to.have.property("tags").that.is.an("array");
        }));

        if (item?.tags && item?.tags.length > 0) {
          item?.tags.forEach((tag, tagIndex) => {
            messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.items[${i}].tags[${tagIndex}]' should have properties 'descriptor', and 'list'`, function () {
              expect(tag).to.have.property("descriptor").that.is.an("object");
              expect(tag).to.have.property("list").that.is.an("array");
            }));

            messageTestSuite.addTest(new Mocha.Test(`Verify the presence of'message.intent.provider.items[${i}].tags[${tagIndex}].descriptor' should have a property named 'code' which is a string`, function () {
              expect(tag.descriptor).to.have.property("code").that.is.a("string");
            }));
            // messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.items[${i}].tags[${tagIndex}].descriptor' should have a property named 'name' which is a string`,function () {
            //             expect(tag.descriptor).to.have.property("name").that.is.a("string");
            //         }));

            messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.items[${i}].tags[${tagIndex}].list' should be a non-empty array`, function () {
              expect(tag.list).to.be.an("array").that.is.not.empty;
            }));

            tag.list.forEach((listItem, listItemIndex) => {
              messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.intent.provider.items[${i}].tags[${tagIndex}].list[${listItemIndex}]' should have properties 'descriptor' and 'value'`, function () {
                expect(listItem).to.have.property("descriptor").that.is.an("object");
                expect(listItem).to.have.property("value").that.is.a("string");
              }));
              messageTestSuite.addTest(new Mocha.Test(` Verify the presence of'message.intent.provider.items[${i}].tags[${tagIndex}].list[${listItemIndex}].descriptor' should have a property named 'code' which is a string`, function () {
                expect(listItem.descriptor).to.have.property("code").that.is.a("string");
              }));
              messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.intent.provider.items[${i}].tags[${tagIndex}].list[${listItemIndex}].descriptor.code' should be equal to '${listItem.descriptor.code}'`, function () {
                expect(listItem.descriptor.code).to.equal(listItem.descriptor.code);
              }));
              messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.intent.provider.items[${i}].tags[${tagIndex}].list[${listItemIndex}].value' should be a string that is not empty`, function () {
                expect(listItem.value).to.be.a("string").that.is.not.empty;
              }));
            });
          })
...
function fulfillmentsTests(message, messageTestSuite, constants) {
  try {
    ...
    if (
      ...
      message?.order?.fulfillments.forEach((it, i) => {
        ...
        if (it.customer) {
          messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.fulfillments[${i}].customer' should be a object`, function () {
            expect(it.customer).to.be.a("object");
          }));
          messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.fulfillments[${i}].customer.person' should be a object`, function () {
            expect(it.customer.person).to.be.a("object");
          }));
          ...
          messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.fulfillments[${i}].customer.contact' should be a object`, function () {
            expect(it.customer.contact).to.be.a("object");
          }));
          ...
          if (it.customer.contact.mail) {
            messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.fulfillments[${i}].customer.contact.mail' should be a string`, function () {
              expect(it.customer.contact.mail).to.be.a("string");
            }));
          }
        }

        if (it?.tags) {
          messageTestSuite.addTest(new Mocha.Test(`Verify the presence of'message.order.fulfillments[${i}].tags' should have a property named 'tags' which is an array`, function () {
            expect(it).to.have.property("tags").that.is.an("array");
          }));

          const arr = [{ code: "NOMINEE_DETAILS" }];
          arr.forEach((ele) => {
            if (it?.tags) {
              const tagIndex = it.tags.findIndex((tag) => tag.descriptor.code === ele.code);
              const tagItem = it.tags[tagIndex];
              ...
              if (tagIndex !== -1) {
                messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}]' should have properties named 'descriptor', 'display' and 'list'`, function () {
                  expect(tagItem).to.have.property("descriptor").that.is.an("object");
                  expect(tagItem).to.have.property("list").that.is.an("array");
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}].descriptor' should have a property named 'code' which is a string`, function () {
                  expect(tagItem.descriptor).to.have.property("code").that.is.a("string");
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}].descriptor.code' should be equal to '${ele.code}'`, function () {
                  expect(tagItem.descriptor.code).to.be.equal(ele.code);
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}].list' should be a non empty array`, function () {
                  expect(tagItem.list).to.be.an("array").that.is.not.empty;
                }));

                const nomineeDetailsArr1 = [{ code: "NOMINEE_NAME" }];

                const nomineeDetailsArr2 = [{ code: "NOMINEE_NAME" }, { code: "NOMINEE_DOB" }, { code: "APPOINTEE_NAME" }, { code: "APPOINTEE_DOB" }, { code: "GOOD_HEALTH_DECLARATION" }];
                  ...
                  case "NOMINEE_DETAILS":
                    switch (constants?.insurance_category) {
                      case "MICRO_TRANSIT_INSURANCE":
                        array = nomineeDetailsArr1;
                        break;
                      case "MICRO_HOSPICASH_INSURANCE":
                        array = nomineeDetailsArr2;
                        break;
                    }
                  default:
                    break;
                ...
                if (array) {
                  array.forEach((it) => {
                    ...
                    if (listItemIndex !== -1) {
                      messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}].list[${listItemIndex}]' should have properties named 'descriptor' and 'value' which are strings`, function () {
                        expect(listItem).to.have.property("descriptor").that.is.an("object");
                        expect(listItem).to.have.property("value").that.is.a("string");
                      }));

                      messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}].list[${listItemIndex}].descriptor' should have properties named 'code' which is a string`, function () {
                        expect(listItem.descriptor).to.have.property("code").that.is.a("string");
                      }));

                      messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${i}].tags[${tagIndex}].list[${listItemIndex}].descriptor.code' should be equal to '${it.code}'`, function () {
...
function providerWithID(message, messageTestSuite) {
  try {
    ...
    messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.provider.id' should be a string`,
      function () {
...
function paymentsCommonTests(message, messageTestSuite) {
  try {
    // const buyerFinderFeeAndSettlementTermsObj = message?.order?.payments.find(
    ...
    message?.order?.payments.forEach((payment, index) => {
      ...
      //         expect(payment.params).to.includes.all.keys("amount", "currency", "transaction_id", "bank_code", "bank_account_number", "virtual_payment_address");
      //       }));
      ...
      //     } else {
      //       messageTestSuite.addTest(new Mocha.Test(`'message.order.payments[${index}].params' should include all the required keys (OPTIONAL)`, function () {
      //         expect(payment.params).to.includes.all.keys("amount", "currency", "bank_code", "bank_account_number", "virtual_payment_address");
      //       }));
      //     }
      ...
      //       { code: "DELAY_INTEREST", name: "delay interest" },
      //       { code: "STATIC_TERMS", name: "static terms" },
      //       { code: "SETTLEMENT_AMOUNT", name: "settlement amount" },
      //       { code: "SETTLEMENT_TYPE", name: "settlement type" },
      ...
      //       { code: "BUYER_FINDER_FEES_TYPE", name: "buyer finder fee type" },
      //       { code: "BUYER_FINDER_FEES_PERCENTAGE" },
      //     ];
      //     const buyerFinderFeeAmountArr = [
      //       { code: "BUYER_FINDER_FEES_TYPE", name: "buyer finder fee type" },
...
function quoteCommonTests(message, messageTestSuite) {
  try {
    ...
    arr.forEach((ele) => {
      const breakupIndex = message?.order?.quote?.breakup.findIndex(
        (breakup) => breakup?.title === ele.title
      );
