const { search, select, init, cancel, confirm, status, update, track, info, rating, catalog_rejection, on_search_response } = require("./index");
...
module.exports = function testRunnerRetailINVL(givenTest, logs, domain, type = "") {
    try {
        ...
        const testFunctions = givenTest.flow
            .map((currentStep) => {
                if (isInventoryLess && (currentStep.action === "select" || currentStep.test === "select")) {
                    return null;
                }
                ...
                console.log("currentTest",JSON.stringify(currentStep,particularLogs));
                ...
                switch (currentStep.test) {
                    ...
                    case "status":
                        if (particularLogs?.request)
                            return () => status(particularLogs?.request, logs, constants);
                        return () => status({}, logs, constants);
                    case "cancel":
