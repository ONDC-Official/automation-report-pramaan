...
function itemsCommonFieldsTests(item, index, messageTestSuite) {
    try {
        ...
        messageTestSuite.addTest(new Mocha.Test(`Verify if 'message.order.items[${index}].price.value' is a upto two decimal place only`, function () {
            expect(item.price.value).to.be.a("string");

            let pattern = /^[+-]?([0-9]*[.])?[0-9]+$/;
            expect(item.price.value).to.match(pattern, `'price.value' does not match the required pattern`);
        }));
        ...
        if (item?.tags && item?.tags.length > 0) {
            ...
            itemArr.forEach((ele) => {
                ...
                messageTestSuite.addTest(new Mocha.Test(`should validate the contents of 'message.order.items[${index}].tags[${tagIndex}]`, function () {
                ...
                messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].descriptor' should have properties 'code' and 'name'`, function () {
                    expect(tag.descriptor.code).to.exist.and.to.be.a("string");
                    expect(tag.descriptor.name).to.exist.and.to.be.a("string");
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].descriptor.code' should be equal to 'LOAN_INFO'`, function () {
                    expect(tag.descriptor.code).to.equal(ele.code);
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].descriptor.name' should be equal to 'Loan Information'`, function () {
                    expect(tag.descriptor.name).to.be.a("string");
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].display' should be 'boolean'`, function () {
                ...
                if (tag?.list && tag?.list.length > 0) {
                    ...
                    arr.forEach((ele) => {
                        const listItemIndex = tag?.list.findIndex((listItem) => listItem?.descriptor?.code === ele.code);
                        const listItem = tag?.list[listItemIndex];

                        messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].list[${listItemIndex}]' (${listItem.descriptor.code}) should exist in the array`, function () {
                            expect(listItemIndex).to.not.equal(-1);
                        }));

                        if (listItemIndex !== -1) {
                            const listItem = tag?.list[listItemIndex];

                            messageTestSuite.addTest(new Mocha.Test(`should validate the contents of 'message.order.items[${index}].tags[${tagIndex}].list[${listItemIndex}]' (${listItem.descriptor.code})`, function () {
                            ...
                            messageTestSuite.addTest(new Mocha.Test(`should validate the contents of 'message.order.items[${index}].tags[${tagIndex}].list[${listItemIndex}].descriptor' (${listItem.descriptor.code})`, function () {
                                expect(listItem.descriptor).to.include.all.keys("code", "name");
                            ...
                            messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].list[${listItemIndex}].descriptor.code' should be '${ele.code}'`, function () {
                                expect(listItem.descriptor.code).to.be.equal(ele.code);
                            }));

                            messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].list[${listItemIndex}].descriptor.name' should be a string (${ele.code})`, function () {
                                expect(listItem.descriptor.name).to.be.a("string");
                            }));

                            messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].tags[${tagIndex}].list[${listItemIndex}].value' should be a string (${ele.code})`, function () {
                                expect(listItem.value).to.be.a("string");
                            }));
                        }
                    })
        ...
        if (message?.order?.fulfillments && message?.order?.fulfillments.length > 0) {
            message.order.fulfillments.forEach((fulfillment, index) => {
                ...
                if (message?.order?.items.find((item) => item?.descriptor?.code.includes("PERSONAL_LOAN"))) {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer.person' should have property 'name'`, function () {
                        expect(fulfillment.customer.person).to.have.property("name").that.is.a("string");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer.person' should have properties 'phone' and 'email'`, function () {
                        expect(fulfillment.customer.contact).to.have.property("phone").that.is.a("string");
                        expect(fulfillment.customer.contact).to.have.property("email").that.is.a("string").and.that.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/);
                    }));
                } else {
                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer' should a property 'organisation'`, function () {
                        expect(fulfillment.customer).to.have.property("organisation");
                        expect(fulfillment.customer.organisation).to.exist.and.to.be.an("object");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`should verify the contents of 'message.order.fulfillments[${index}].customer.organisation'`, function () {
                        expect(fulfillment.customer.organisation).to.include.all.keys("address", "state", "city", "contact");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer.organisation.address' should be a string`, function () {
                        expect(fulfillment.customer.organisation.address).to.be.a("string");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer.organisation.state' should have property 'name' that is a string`, function () {
                        expect(fulfillment.customer.organisation.address).to.have.property("name").that.is.a("string");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer.organisation.city' should have properties 'name' and 'code'`, function () {
                        expect(fulfillment.customer.organisation.city).to.have.property("name").that.is.a("string");
                        expect(fulfillment.customer.organisation.city).to.have.property("code").that.is.a("string");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].customer.organisation.contact' should have properties 'phone' and 'email'`, function () {
                        expect(fulfillment.customer.organisation.contact).to.have.property("phone").that.is.a("string");
                        expect(fulfillment.customer.organisation.contact).to.have.property("email").that.is.a("string").and.that.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/);
                    }));
                }
                ...
                messageTestSuite.addTest(new Mocha.Test(`'message.order.fulfillments[${index}].state.descriptor.code' should be a string and equal to '${loanState.code}'`, function () {
...
function quoteTests(message, messageTestSuite) {
    try {
        ...
        arr.forEach((ele) => {
            ...
            if (breakUpItem?.title === "OTHER_UPFRONT_CHARGES") {
                messageTestSuite.addTest(new Mocha.Test(`'message.order.quote.breakup[${breakUpItemIndex}]' (${ele.code}) should have 'title' and 'price' properties (OPTIONAL)`, function () {
                    expect(breakUpItem).to.have.property('title').that.is.a('string');
                    expect(breakUpItem).to.have.property('price').that.is.an('object');
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.order.quote.breakup[${breakUpItemIndex}].title' should be equal to '(${ele.code})' (OPTIONAL)`, function () {
            ...
            message?.order?.cancellation_terms.forEach((ele, index) => {
                if (ele?.fulfillment_state) {
                    if (ele?.fulfillment_state?.descriptor?.code === "SANCTIONED") {
                        ...
                        messageTestSuite.addTest(new Mocha.Test(`'message.order.cancellation_terms[${index}].fulfillment_state.descriptor' should have a property 'code' that is equal to "SANCTIONED"`, function () {
                ...
                if (ele?.external_ref) {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`'message.order.cancellation_terms[${index}].external_ref' should have property 'mimetype' and 'url'`, function () {
