...
async function on_update({ context, message } = {}, testCaseId, step) {
    const testSuite = new Mocha.Suite(`on_update ${step ? step : ""} Request Verification `)
    try {
        ...
        if (message?.order?.items && message.order.items.length > 0) {
            message?.order?.items.forEach((item, index) => {
                ...
                messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].category_ids' should be an array, that should not be empty`, function () {
                    expect(item.category_ids).to.be.an('array').that.is.not.empty;
                }));
                if (item?.category_ids && item?.category_ids.length > 0) {
                    item.category_ids.forEach((categoryID, categoryIDIndex) => {
                        messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].category_ids[${categoryIDIndex}]' should be string`, function () {
                            expect(categoryID).to.be.a("string");
                        }));
                    })
                };
                messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].fulfillment_ids' should be an array, that should not be empty`, function () {
                ...
                if (item?.fulfillment_ids && item?.fulfillment_ids.length > 0) {
                    item.category_ids.forEach((fulfillmentID, fulfillmentIDIndex) => {
                        messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${index}].fulfillment_ids[${fulfillmentIDIndex}]' should be string`, function () {
        ...
        if (message?.order?.payments && message?.order?.payments.length > 0) {
            ...
            // const missedEMIPayments = message?.order?.payments.filter(payment => payment?.time?.label === "MISSED_EMI_PAYMENT");

            // if (missedEMIPayments && missedEMIPayments.length > 0) {
            //     messageTestSuite.addTest(new Mocha.Test(`should have atleast one missed EMI payment`, function () {
            //         expect(missedEMIPayments).to.be.an("array").that.is.not.empty;
            //     }));
            // }
            ...
            //         messageTestSuite.addTest(new Mocha.Test(`'message.order.payments[${index}]' should have a valid 'status' should be 'NOT-PAID' or 'PAID' or 'DELAYED' or 'DEFERRED' `, function () {
            //             expect(payment).to.have.property('status').that.is.a('string').and.is.oneOf(['DELAYED', 'PAID', 'NOT-PAID', 'DEFERRED']);

            //         }));

            //         if (payment?.time?.label === "MISSED_EMI_PAYMENT") {
            ...
            //                 expect(payment.time).to.have.property('label').that.is.a('string').and.equals('MISSED_EMI_PAYMENT');
            //             }));
            //         } else {
