            ...
            message?.catalog?.providers.forEach((provider, index) => {
                ...
                if (provider?.items && provider?.items.length > 0) {
                    provider?.items.forEach((item, i) => {
                        ...
                        const infoArr = [{ code: "INFO", name: "information" }];
                        ...
                        infoArr.forEach((ele) => {
                            ...
                            if (tagIndex !== -1) {
                                ...
                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].items[${i}].tags[${tagIndex}]' should have a property named 'display' that is a 'boolean' `, function () {
                        ...
                        if (item?.xinput?.head?.headings && item?.xinput?.head?.headings.length > 0) {
                            item?.xinput?.head?.headings.forEach((heading, l) => {
                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].items[${i}].xinput.head.headings[${l}]' should be equal to 'Organization Information'`, function () {
                                    expect(heading).to.be.oneOf(["Personal Information", "Organization Information"]);
                                }));
                            });
                        }
                        ...
                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].items[${i}].xinput.xinput' should be a 'boolean' `, function () {
                ...
                if (provider?.payments && provider?.payments.length > 0) {
                    provider?.payments.forEach((payment, index) => {
                        ...
                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags' should be an array with length not equal 0`, function () {
                        ...
                        if (payment?.tags && payment?.tags.length > 0) {
                            ...
                            arr.forEach((ele, i) => {
                                ...
                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags' should have an '${ele.name}' object`, function () {
                                    expect(tag).to.exist.and.to.be.an("object");
                                }));

                                messageTestSuite.addTest(new Mocha.Test(`should verify the contents of 'message.catalog.providers[${index}].tags[${eleIndex}]'`, function () {
                                    expect(tag).to.include.all.keys("descriptor", "display", "list");
                                }));

                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].descriptor' should have property 'code'`, function () {
                                    expect(tag.descriptor).to.have.property("code");
                                }));

                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].descriptor.code' should be ${ele.code}`, function () {
                                    expect(tag.descriptor.code).to.equal(ele?.code);
                                }));

                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].display' should be a 'boolean' `, function () {
                                    expect(tag.display).to.be.a('boolean');
                                }));

                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].list' should be an array that is not empty`, function () {
                                ...
                                if (tag?.list && tag.list.length > 0) {
                                    ...
                                    array.forEach((it) => {
                                        ...
                                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].list' should have an '${it.name}' object`, function () {
                                            expect(listItem).to.exist.and.to.be.an("object");
                                        }));

                                        messageTestSuite.addTest(new Mocha.Test(`should verify the contents of 'message.catalog.providers[${index}].tags[${eleIndex}].list[${itIndex}]' (${it.code})`, function () {
                                            expect(listItem).to.include.all.keys("descriptor", "value");
                                        }));

                                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].list[${itIndex}].descriptor' should have property 'code' (${it.code})`, function () {
                                            expect(listItem.descriptor).to.have.property("code");
                                            expect(listItem.descriptor.code).to.exist;
                                        }));

                                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.providers[${index}].tags[${eleIndex}].list[${itIndex}].value' should have the expected data type (${it.code})`, function () {
                                            switch (it?.code) {
...
async function on_search({ context, message } = {},logs = [],constants ={}) {
    const testSuite = new Mocha.Suite(`on_search (I) Request Verification`);
