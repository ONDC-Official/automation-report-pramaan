const on_search = require("../v2.0.0/on_search");
const {
    on_search_1,
    on_search_2,
    on_search_3,
        ...
        let testFunctions = givenTest.flow
            .map((currentStep, index) => {
                ...
                switch (currentStep.test) {
                    case "on_search_one":
                        const searchOne = particularLogs[searchIndex[currentStep.test]];
                        if (searchOne)
                            return () => on_search_1(searchOne.request,logs,constants);
                        return () => on_search_1({},logs,constants);
                    case "on_search_two":
                        if (aa_mandatory) {
                            const searchTwo = particularLogs[searchIndex[currentStep.test]];
                            if (searchTwo)
                                return () => on_search_2(searchTwo.request,logs,constants);
                            return () => on_search_2({},logs,constants);
                        }
                    case "on_search_three":
                        const searchThree = particularLogs[searchIndex[currentStep.test] - searchLogIndex];
                        if (searchThree)
                            return () => on_search_3(searchThree.request, romanIndex[searchIndex[currentStep.test] + 1 - searchLogIndex],logs,constants);
                        return () => on_search_3({}, romanIndex[searchIndex[currentStep.test] + 1 - searchLogIndex],logs,constants);
                    case "on_select_one":
                    case "on_select_two":
