...
async function missedEMIPaymentTests({ context, message } = {}) {
    try {
        const testSuite = new Mocha.Suite("on_update Request Verification (Missed EMI Payment)")
            ...
            message?.order?.payments.forEach((payment, index) => {
                if (payment?.tags) {
                    ...
                    if (payment?.time?.label === "MISSED_EMI_PAYMENT") {
                        messageTestSuite.addTest(new Mocha.Test(`'message.order.payments[${index}].time' should have a valid 'label' property (OPTIONAL)`, function () {
...
module.exports = {
    on_updateMissedEmiPayment: missedEMIPaymentTests
}