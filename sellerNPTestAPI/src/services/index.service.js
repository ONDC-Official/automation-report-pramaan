...
async function downloadHTMLReport(req, res) {
  try {
    ...
    let reports = await database.getReports(test_id);

    // Sort the reports based on the orderArray
    reports.sort((a, b) => {
    ...
    if (jsonReports.length > 0) {
      ...
      mergedReport.results.sort((a, b) => {
        const aIndex = orderArray.indexOf(a.flow_id);
        const bIndex = orderArray.indexOf(b.flow_id);
...
async function downloadLogs(req, res) {
  try {
    ...
    const { transaction_id, id } = req.body;

    // Assuming getLogs function fetches logs based on the transaction ID
    const allLogs = await database.getLogs(transaction_id);
    const sellerLogs = allLogs.filter((ele) => {
      if (
        ele?.request?.context?.bpp_id &&
        ele?.request?.context?.bpp_id === id
      ) {
        return true;
      }

      if (!ele?.request?.context?.bpp_id) {
        return true;
      }
    ...
    // Create an empty object to store logs
    let logsArray = {};

    // Iterate over the log flow to structure logs
    ...
    const search = sellerLogs.find((ele) => ele.action === "search");
...
async function downloadLogsBrowser(req, res) {
  try {
    ...
    // Assuming getLogs function fetches logs based on the transaction ID
    const sellerLogs = await database.getLogs(transaction_id);
  ...
  try {
    ...
    if (jsonReports.length > 0) {
      ...
      await generator.create(mergedReport, {
        reportDir: reportDir,
        reportTitle: `Pramaan Test Report ID: ${test_id} generated at: ${JSON.stringify(
          new Date(Date.now(reports[0]?.created_at))
        )}`,
        reportPageTitle: `${test_id}_report`,
        reportFilename: `${test_id}_report`,
        overwrite: true,
...
async function getJSONReport(req, res) {
  try {
    ...
    const jsonReports = reports.map((item) => JSON.parse(item?.report));
    if (jsonReports.length > 0) {
      return res.status(200).json(jsonReports);
    }
