    ...
    switch (compliance) {
        ...
        case 'gps-coord':
            test = new Mocha.Test(`'[id: ${property?.id}_compliance]' ${testName} should be valid GPS coordinates with at least four decimal places${OPTIONAL}`, function () {
                const gpsPattern = /^-?\d{1,3}\.\d{4,},-?\d{1,3}\.\d{4,}$/;
...
function contextTests(context = {}, constants = {}, logs = [], schema) {
    try {
        ...
        validSchema.required.forEach((prop, index) => {
            contextTestSuite.addTest(new Mocha.Test(`[id: ${domain}_bpp_context_${15 + index}] 'context' should have required property '${prop}'`, function () {
