const { updateTrail } = require("../centralizedUtilities/trailManagement");
const {
    on_search,
    on_select,
    on_init,
...
const onStatusEnumMap = {
    ...
    "on_update_return_liquidated": "Liquidated",
...
module.exports = function testRunnerRetail(givenTest, logs, domain, type = "") {
    ...
    let onConfirmLog = {};
    const step = givenTest.flow.map((flowIndex) => {
        return flowIndex.test
    })
    const constants = {
        core_version: "1.2.0",
        domain: domain,
            ...
            .map((currentStep) => {
                ...
                switch (currentStep.test) {
                    ...
                    case "on_select_out_of_stock":
                        if (particularLogs?.request)
                            return () => on_select(particularLogs?.request, logs, currentTrail, constants, currentStep?.test);
                        return () => on_select({}, logs, currentTrail, constants, currentStep?.test);
                    case "on_init":
                        if (particularLogs?.request)
                            return () => on_init(particularLogs?.request, logs, currentTrail, constants);
                        return () => on_init({}, logs, currentTrail, constants);
                    case "on_confirm":
                        if (particularLogs?.request)
                            return () => on_confirm(particularLogs?.request, logs, currentTrail, constants);
                        return () => on_confirm({}, logs, currentTrail, constants);
                    case "on_track":
                    ...
                    case "on_cancel_rto_initiated":
                        if (particularLogs?.request)
                            return () => on_cancel(particularLogs?.request, logs, constants);
                        return () => on_cancel({}, logs, constants);
                    ...
                    case "on_update_return_liquidated":
