...
async function on_status({ context, message } = {}, descriptor) {
    try {
        const testSuite = new Mocha.Suite("on_status Request Verification ");

        testSuite.addSuite(contextTests(context, "on_status"));
        ...
        switch (descriptor) {
            case 'HEALTH_INSURANCE':
                on_status_insurance(message, messageTestSuite);
                break;
            default:
...
async function on_status_insurance(message, messageTestSuite) {
    try {
