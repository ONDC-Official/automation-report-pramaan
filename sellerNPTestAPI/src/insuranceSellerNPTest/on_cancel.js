...
async function on_cancel({ context, message } = {}, descriptor) {
    try {
        const testSuite = new Mocha.Suite(`on_cancel  Request Verification`);

        testSuite.addSuite(contextTests(context, "on_cancel"));
        ...
        switch (descriptor) {
            case 'HEALTH_INSURANCE':
                on_cancel_insurance(message, messageTestSuite);
                break;
         case 'MARINE_INSURANCE':
                marineOn_cancel(message, messageTestSuite);
                break;
            case 'MOTOR_INSURANCE':
                motorOn_cancel(message, messageTestSuite);
                break;        
            default:
...
async function on_cancel_insurance(message, messageTestSuite, step) {
    try {
...
async function marineOn_cancel(message, messageTestSuite, step) {
    try {
        messageTestSuite.addTest(new Mocha.Test("should verify the contents of 'message.order'", function () {
            expect(message.order).to.include.all.keys("id", "provider", "items", "quote", "fulfillments", "payments", "documents");
        }));
...
async function motorOn_cancel(message, messageTestSuite, step) {
    try {
        messageTestSuite.addTest(new Mocha.Test("should verify the contents of 'message.order'", function () {
            expect(message.order).to.include.all.keys("id", "provider", "items", "quote", "fulfillments", "payments", "documents");
        }));
