...
async function on_status({ context, message } = {}, logs = [], type = "", state = "") {
    try {
        ...
        const updated_at = onConfirmLog?.message?.order?.updated_at;
        ...
        if (message?.order?.fulfillments && message?.order?.fulfillments?.length > 0) {
            message?.order?.fulfillments.forEach((fulfillment, fulfillmentIndex) => {
                ...
                messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].type' which is a string (OPTIONAL)`, function () {
                    ...
                    fulfillment?.stops.forEach((stopItem, stopIndex) => {
                        ...
                        if (fulfillment?.type === "TICKET") {
                            if (stopIndex === 0) {
                                ...
                                switch (state) {
                                    case "COMPLETEDD":
                                        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].stops[${stopIndex}].authorization.status' which is a string should be "CLAIMED" (OPTIONAL)`, function () {
                                            expect(stopItem.authorization.status).to.exist.and.to.be.a("string").and.to.be.oneOf(["CLAIMED"]);
                                        }));
                                        break;
                                    default:
        ...
        messageTestSuite.addTest(new Mocha.Test("'message.order.updated_at' should exist, be a string, and follow ISO 8601 format", function () {
            ...
            // expect(message.order.updated_at).to.deep.equal(updated_at);
        }));
