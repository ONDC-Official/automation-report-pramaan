...
function addRetinvlPaymentTests(messageTestSuite, message, constants = {}) {
    if (constants?.domain !== "ONDC:RETINVL") {
        return;
    }
    ...
    providers.forEach((provider, providerIndex) => {
        const fulfillmentIds = provider?.fulfillments?.map((fulfillment) => fulfillment?.id).filter(Boolean) || [];
        const providerPaymentIds = provider?.payments?.map((payment) => payment?.id).filter(Boolean) || [];

        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments' should be a non-empty array`, function () {
            expect(provider?.payments).to.be.an("array").that.is.not.empty;
        }));

        if (Array.isArray(provider?.payments)) {
            provider.payments.forEach((payment, paymentIndex) => {
                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}]' should be an object`, function () {
                    expect(payment).to.exist.and.to.be.an("object");
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].id' should be a non-empty string`, function () {
                    expect(payment?.id).to.exist.and.to.be.a("string").that.is.not.empty;
                }));

                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].type' should be a valid payment type`, function () {
                    expect(payment?.type).to.be.a("string").and.to.be.oneOf(["ON-ORDER", "ON-FULFILLMENT", "PART-PAYMENT"]);
                }));

                if (payment?.type === "PART-PAYMENT") {
                    const paymentDetailTagIndex = payment?.tags?.findIndex((tag) => tag?.code === "payment_detail") ?? -1;
                    const paymentDetailTag = payment?.tags?.[paymentDetailTagIndex];

                    messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].tags' should be a non-empty array for PART-PAYMENT`, function () {
                        expect(payment?.tags).to.be.an("array").that.is.not.empty;
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].tags' should contain 'payment_detail'`, function () {
                        expect(paymentDetailTag).to.exist.and.to.be.an("object");
                    }));

                    if (paymentDetailTagIndex !== -1) {
                        const allowedFulfillmentIndex = paymentDetailTag?.list?.findIndex((listItem) => listItem?.code === "ALLOWED-FULFILLMENTS") ?? -1;
                        const allowedFulfillmentItem = paymentDetailTag?.list?.[allowedFulfillmentIndex];

                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].tags[${paymentDetailTagIndex}].list' should be a non-empty array`, function () {
                            expect(paymentDetailTag?.list).to.be.an("array").that.is.not.empty;
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].tags[${paymentDetailTagIndex}].list' should contain 'ALLOWED-FULFILLMENTS'`, function () {
                            expect(allowedFulfillmentItem).to.exist.and.to.be.an("object");
                        }));

                        if (allowedFulfillmentIndex !== -1) {
                            const allowedFulfillmentIds = typeof allowedFulfillmentItem?.value === "string"
                                ? allowedFulfillmentItem.value.split(",").map((fulfillmentId) => fulfillmentId.trim()).filter(Boolean)
                                : [];

                            messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].tags[${paymentDetailTagIndex}].list[${allowedFulfillmentIndex}].value' should be a non-empty string`, function () {
                                expect(allowedFulfillmentItem?.value).to.be.a("string").that.is.not.empty;
                            }));

                            messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}].tags[${paymentDetailTagIndex}].list[${allowedFulfillmentIndex}].value' should resolve to at least one fulfillment id`, function () {
                                expect(allowedFulfillmentIds).to.be.an("array").that.is.not.empty;
                            }));

                            allowedFulfillmentIds.forEach((fulfillmentId, fulfillmentIndex) => {
                                messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].payments[${paymentIndex}]' allowed fulfillment '${fulfillmentId}' at index ${fulfillmentIndex} should exist in provider.fulfillments`, function () {
                                    expect(fulfillmentIds).to.include(fulfillmentId);
                                }));
                            });
                        }
                    }
        ...
        provider?.items?.forEach((item, itemIndex) => {
            messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].items[${itemIndex}].payment_ids' should be a non-empty array`, function () {
            ...
            if (Array.isArray(item?.payment_ids)) {
                item.payment_ids.forEach((paymentId, paymentIdIndex) => {
                    messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].items[${itemIndex}].payment_ids[${paymentIdIndex}]' should be a non-empty string`, function () {
                        expect(paymentId).to.be.a("string").that.is.not.empty;
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`'message.catalog.bpp/providers[${providerIndex}].items[${itemIndex}].payment_ids[${paymentIdIndex}]' should exist in provider.payments`, function () {
                        expect(providerPaymentIds).to.include(paymentId);
                    }));
                });
            }
        });
...
function onSearchMessageTests({ context, message } = {}, logs = [], constants = {}) {
    try {
        ...
        if (constants?.flow === "RET_ENH_001" || constants?.flow === "RET_ENH_O1E" || constants?.flow === "RET_ENH_00A" || constants?.flow === "RET_ENH_009_FREEBIE" || constants?.flow === "RET_ENH_009_DISCOUNT" || constants?.flow === "RET_ENH_009_COMBO" || constants?.flow === "RET_ENH_009_SLAB" || constants?.flow === "RET_ENH_009_BUYXGETY_B" || constants?.flow
            ...
            addRetinvlPaymentTests(messageTestSuite, message, constants);
            const providers = message?.catalog['bpp/providers'] || [];
            if (message?.catalog['bpp/providers'] && message?.catalog['bpp/providers'].length > 0) {
                message?.catalog['bpp/providers'].forEach((provider, providerIndex) => {
