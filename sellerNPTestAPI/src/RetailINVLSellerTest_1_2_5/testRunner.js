...
function findAppropriateOnStatus(logs = [], action = "", type = "", key = "", skip = 0) {
    ...
    console.log("findAppropriateOnStatus", logs, action, type, key, skip);
    

    let matchCount = 0;
    for (const log of logs) {
        console.log("Checking log", log?.action, JSON.stringify(log?.request?.message?.order?.fulfillments));
        ...
        console.log("okkkkk",JSON.stringify(ok));
...
function getOnStatusStep(log = {}) {
    return log?.request?.message?.order?.fulfillments?.find(
        (fulfillment) => fulfillment?.state?.descriptor?.code
    )?.state?.descriptor?.code || "on_status";
}


module.exports = function testRunnerRetailINVL(givenTest, logs, domain, type = "") {
    const flowId = givenTest?.id;
    const isInventoryLess = givenTest?.type === "RETINVL" || domain === "ONDC:RETINVL";
    let onConfirmLog = {};
    let statusIndex = 0;
    ...
    try {
        const testFunctions = givenTest.flow
            .map((currentStep) => {
                if (isInventoryLess && (currentStep.action === "on_select" || currentStep.test === "on_select" || currentStep.test === "on_select_out_of_stock")) {
                    return null;
                }
                ...
                switch (currentStep.test) {
                    ...
                    case "on_cancel_forced":
                        ...
                        return () => on_cancel({}, currentStep.test, logs, constants);
                    case "on_status":
                        const onStatusLog = particularLogs[statusIndex++];
                        if (onStatusLog?.request) {
                            return () => on_status(onStatusLog.request, getOnStatusStep(onStatusLog), logs, constants);
                        }
                        return () => on_status({}, currentStep.test, logs, constants);
                    ...
                    case "on_status_delivered":
                        ...
                        console.log("on_status_forward_log", on_status_forward_log);
