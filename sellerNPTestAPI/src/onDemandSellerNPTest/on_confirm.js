...
async function on_confirm(log = {}, driverState = "", isSelfPickup = false, logs = [], constants = {}) {
    ...
    try {
        ...
        if (driverState === "DRIVER_NOT_FOUND") {
            ...
            const errorTestSuite = Mocha.Suite.create(testSuite, "Verification of Error");

            errorTestSuite.addTest(new Mocha.Test("Verify the presence of 'error' which is an object", function () {
                expect(error).to.exist;
            }));

            errorTestSuite.addTest(new Mocha.Test("'error' should have a property named 'code' that is a string", function () {
                expect(error).to.have.property('code').that.is.a("string");
            }));

            errorTestSuite.addTest(new Mocha.Test("'error.code' should be equal to '90203'", function () {
                expect(error.code).to.equal('90203');
            }));

            errorTestSuite.addTest(new Mocha.Test("'error' should have a property named 'message' that is a string", function () {
        ...
        // In case someone assigns a driver for a normal confirm in any of the flows
        if (driverState === "NORMAL" && message?.order?.fulfillments[0]?.state?.descriptor?.code === "RIDE_ASSIGNED") {
        ...
        if (message?.order?.cancellation_terms && message?.order?.cancellation_terms.length > 0) {
            message.order.cancellation_terms.forEach((C_terms, C_termsIndex) => {
                ...
                if (C_terms?.cancellation_fee?.amount) {
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.cancellation_terms[${C_termsIndex}].cancellation_fee.amount' and that it is an object (OPTIONAL)`, function () {
        ...
        if (message?.order?.fulfillments && message?.order?.fulfillments.length > 0) {
            message.order.fulfillments.forEach((fulfillment, fulfillmentIndex) => {
                ...
                if (driverState === "ASSIGNED") {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].agent.contact' which is an object`, function () {
                        expect(fulfillment.agent.contact).to.exist.and.to.be.an("object");
                    }));

                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].agent.contact.phone' which is a string and valid phone number (OPTIONAL)`, function () {
                        const phoneValue = fulfillment.agent.contact.phone;

                        const phonePattern = /^[0-9]{10}$/;
                        expect(phoneValue).to.exist.and.to.be.a("string");
                        expect(phoneValue).to.match(phonePattern, "Phone number must be a valid 10-digit number");
                    }));
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].agent.person.name' which is a string (OPTIONAL)`, function () {
                        expect(fulfillment.agent.person.name).to.exist.and.to.be.a("string");
                    }));
                }
                ...
                if (driverState === "ASSIGNED") {
                    messageTestSuite.addTest(new Mocha.Test(`Verify 'message.order.fulfillments[${fulfillmentIndex}].state.descriptor.code' should equal 'RIDE_ASSIGNED'`, function () {
                        expect(fulfillment.state.descriptor.code).to.equal("RIDE_ASSIGNED");
                    }));
                } else {
                    messageTestSuite.addTest(new Mocha.Test(`Verify 'message.order.fulfillments[${fulfillmentIndex}].state.descriptor.code' should equal 'RIDE_CONFIRMED'`, function () {
                        expect(fulfillment.state.descriptor.code).to.equal("RIDE_CONFIRMED");
                    }));
                }
                ...
                if (fulfillment?.stops && fulfillment?.stops.length > 0) {
                    fulfillment.stops.forEach((stopItem, stopIndex) => {
                        ...
                        if (driverState === "ASSIGNED" && stopIndex === 0) {
                ...
                if (driverState === "ASSIGNED") {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].vehicle.model' which is a string`, function () {
        ...
        if (message?.order?.payments && message?.order?.payments.length > 0) {
            message.order.payments.forEach((payment, z) => {
                ...
                messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.payments[${z}].status' which is a string and is one of ${paymentStatus}`, function () {
                ...
                messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.payments[${z}].type' which is a string`, function () {
                    expect(payment.type).to.be.a("string").and.to.be.oneOf(["PRE-ORDER", "ON-ORDER", "ON-FULFILLMENT", "POST-FULFILLMENT"]);
                }));
        ...
        testSuite.addTest(new Mocha.Test("on_confirm request failed to be verified because of some missing payload or some internal error", function () {
