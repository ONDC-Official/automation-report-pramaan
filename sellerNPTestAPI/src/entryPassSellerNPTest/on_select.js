...
async function on_select({ context, message } = {}, step, isSelfPickup = false, logs = [], constants = {}) {
    ...
    try {
        ...
        if (message?.order?.items && message?.order?.items.length > 0) {
            message?.order?.items.forEach((item, i) => {
                ...
                const arr = [{ code: "INCLUSIONS" }, { code: "EXCLUSIONS" }];
                arr.forEach((ele) => {
                    ...
                    if (tagIndex !== -1) {
                        ...
                        messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${i}].tags[${tagIndex}].descriptor.code' should have be equal to '${ele.code}'${optionalTags}`, function () {
                            expect(tagItem.descriptor.code).to.be.equal(ele.code);
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`'message.order.items[${i}].tags[${tagIndex}].list' should have be a non empty array${optionalTags}`, function () {
                ...
                if (item?.xinput) {
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput' which is a object`, function () {
                        expect(item.xinput).to.exist.and.to.be.a("object");
                    }));
                    if (item?.xinput?.head) {
                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head' should be an object`, function () {
                            expect(item.xinput.head).to.be.an("object");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.descriptor' should be an object`, function () {
                            expect(item.xinput.head.descriptor).to.be.an("object");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.descriptor.name' should be a 'string' (OPTIONAL)`, function () {
                            expect(item.xinput.head.descriptor.name).to.be.a("string");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.index' should be an object`, function () {
                            expect(item.xinput.head.index).to.be.an("object");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.index.min' should be a number (OPTIONAL)`, function () {
                            expect(item.xinput.head.index.min).to.be.a("number");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.index.cur' should be a number(OPTIONAL)`, function () {
                            expect(item.xinput.head.index.cur).to.be.a("number");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.index.max' should be a number (OPTIONAL)`, function () {
                            expect(item.xinput.head.index.max).to.be.a("number");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.head.headings' should be an array (OPTIONAL)`, function () {
                    ...
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form' which is a object`, function () {
                        expect(item.xinput.form).to.exist.and.to.be.a("object");
                    }));
                    messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form.id' which is a string`, function () {
                        expect(item.xinput.form.id).to.exist.and.to.be.a("string");
                    }));
                    if (step === "I") {
                        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form.mime_type' which is a string`, function () {
                            expect(item.xinput.form.mime_type).to.exist.and.to.be.a("string");
                        }));
                        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form.url' which is a string`, function () {
                            expect(item.xinput.form.url).to.exist.and.to.be.a("string");
                        }));
                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.form.resubmit' should be boolean (OPTIONAL)`, function () {
                            expect(item.xinput.form.resubmit).to.be.a("boolean");
                        }));

                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.form.multiple_sumbissions' should be boolean (OPTIONAL)`, function () {
                            expect(item.xinput.form.multiple_sumbissions).to.be.a("boolean");
                        }));
                        messageTestSuite.addTest(new Mocha.Test(`verify that 'message.order.items[${i}].xinput.required' should be true (OPTIONAL)`, function () {
                            expect(item.xinput.required).to.be.true;
                        }));
                    }
                    if (item.xinput.form_response) {
                        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form_response' which is a object`, function () {
                            expect(item.xinput.form_response).to.exist.and.to.be.a("object");
                        }));
                        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form_response.status' which is a string`, function () {
                            expect(item.xinput.form_response.status).to.exist.and.to.be.a("string");
                        }));
                        messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.items[${i}].xinput.form_response.submission_id' which is a string`, function () {
