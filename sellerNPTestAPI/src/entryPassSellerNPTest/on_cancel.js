    ...
    try {
                ...
                if (fulfillment?.stops && fulfillment?.stops.length > 0) {
                    fulfillment.stops.forEach((stopItem, stopIndex) => {
                        ...
                        if (stopItem?.authorization) {
                            ...
                            messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.fulfillments[${fulfillmentIndex}].stops[${stopIndex}].authorization.valid_to' which is a string (OPTIONAL)`, function () {
        ...
        if (message?.order?.payments && message?.order?.payments.length > 0) {
            message.order.payments.forEach((payment, z) => {
                ...
                messageTestSuite.addTest(new Mocha.Test(`Verify the presence of 'message.order.payments[${z}].params.account' which is a string`, function () {
        ...
        messageTestSuite.addTest(new Mocha.Test("Verify the presence of 'message.order.cancellation' which is an object", function () {
            expect(message.order.cancellation).to.exist.and.to.be.an("object");
        }));
        messageTestSuite.addTest(new Mocha.Test("Verify message.order.cancellation.cancelled_by exists and is string", function () {
            expect(message.order.cancellation.cancelled_by).to.exist.and.to.be.a("string");
        }));
        messageTestSuite.addTest(new Mocha.Test("Verify the presence of 'message.order.cancellation.reason' which is an object", function () {
            expect(message.order.cancellation.reason).to.exist.and.to.be.an("object");
        }));
        messageTestSuite.addTest(new Mocha.Test("Verify the presence of 'message.order.cancellation.reason.descriptor' which is an object", function () {
            expect(message.order.cancellation.reason.descriptor).to.exist.and.to.be.an("object");
        }));
        messageTestSuite.addTest(new Mocha.Test("Verify message.order.cancellation.reason.descriptor.code exists and is string", function () {
            expect(message.order.cancellation.reason.descriptor.code).to.exist.and.to.be.a("string");
        }));
