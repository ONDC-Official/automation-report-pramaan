...
const { quoteCommonTests, itemsCommonFieldsTests, providerWithID, xinputForGeneral, fulfillmentsTests, itemsWithXinputTests } = require('./commonTest');
const response_verification = require("../centralizedUtilities/responseVerification");

async function on_select({ context, message } = {}, flowId, logs = [], constants = {}) {
    try {
        ...
        if (message?.order?.items && message?.order?.items.length > 0) {
            message.order.items.forEach((item, i) => {
                ...
                //message.order.item.parent_item_id
        ...
        const arr = [
            ...
            { title: "CONVIENCE_FEE", name: "CONVIENCE_FEE" },
            { title: "TAX", name: "tax" },
                    ...
                    arr.forEach((ele) => {
                        if (it?.customer?.person?.tags) {
                            ...
                            if (tagIndex !== -1) {
                                ...
                                const nomineeDetailsArr = [{ code: "NOMINEE_NAME" }];

                                let array;
                                switch (tagItem?.descriptor?.code) {
                                    case "NOMINEE_DETAILS":
                                        array = nomineeDetailsArr;
                                        break;
