...
async function on_init({ context, message, error } = {}, flowId, logs = [], constants = {}) {
    try {
        const testSuite = new Mocha.Suite("on_init Request Verification");
        const responseTestSuite = response_verification({ context, message }, logs, constants);
        contextTests(context, "on_init", testSuite);

        /** adding tests for Error Flows */
        if (["INS_23"].includes(flowId)) {
            const errorTestSuite = Mocha.Suite.create(testSuite, "Verification of Error");

            errorTestSuite.addTest(new Mocha.Test(`Verify the presence of error in the request`, function () {
            ...
            errorTestSuite.addTest(new Mocha.Test(`'error.code' must be equal to '81201'`, function () {
                expect(error.code).to.equal("81201");
            }))

            testSuite.addSuite(errorTestSuite);
            return testSuite;
        }
