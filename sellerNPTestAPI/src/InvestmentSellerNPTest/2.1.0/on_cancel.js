...
async function on_cancel({ context, message } = {}, step, flowId, logs = []) {
    try {
        const action = "on_cancel";

        const version = logs.map((log) => log?.request?.context?.version);
        ...
        if (!version.includes("2.1.0")) {
            ...
            if (!flowId == "INVESTMENT_3" && !flowId == "INVESTMENT_6" && flowId !== "INVESTMENT_7A" && flowId !== "INVESTMENT_7B" && flowId !== "INVESTMENT_7C" && flowId !== "INVESTMENT_7D") {
            ...
            if (flowId !== "INVESTMENT_7A" && flowId !== "INVESTMENT_7B" && flowId !== "INVESTMENT_7C" && flowId !== "INVESTMENT_7D" && flowId !== "INVESTMENT_6") {
